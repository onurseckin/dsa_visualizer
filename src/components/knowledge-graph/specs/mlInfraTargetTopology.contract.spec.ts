...
describe("target ML infrastructure curriculum topology", () => {
  it("declares all 23 frozen target topics while keeping transitional legacy topics separate", () => {
    ...
    const transitionalLegacyIds = mlCatalogIds.filter(
      (topicId) => !(TARGET_TOPIC_IDS as readonly string[]).includes(topicId),
    );
