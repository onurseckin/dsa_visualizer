...
describe("GraphVisualizer Spec", () => {
  ...
  it("handles edges pointing to missing nodes gracefully without crashing", () => {
    const invalidEdges: GraphEdgeItem[] = [
      { from: "A", to: "GhostNode1" },
      { from: "GhostNode2", to: "B" },
      { from: "A", to: "B", weight: 99 },
    ];

    render(<GraphVisualizer nodes={nodes} edges={invalidEdges} width={800} height={400} />);

    expect(screen.getByText("Node A")).toBeInTheDocument();
    expect(screen.getByText("99")).toBeInTheDocument();
    expect(screen.queryByText("Graph Sample")).toBeNull();
  });
});
