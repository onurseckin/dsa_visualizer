...
describe("TriviaSettingsControls Component Spec", () => {
  ...
  it("keeps both blank counts inside the engine range", () => {
    ...
    fireEvent.change(slider(/hardest level/i), {
      target: { value: String(MAX_BLANKS_CEILING + 500) },
    });
  ...
  it("explains every control in one line", () => {
    ...
    expect(
      screen.getByText(/the drill finishes once every line has been drilled/i),
    ).toBeInTheDocument();
