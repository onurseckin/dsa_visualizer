...
describe('TileTray Component Spec', () => {
  it('renders every candidate as a monospace draggable tile and counts what is left', () => {
    render(<TileTray tiles={TILES} onSelect={vi.fn()} />);
    ...
    expect(screen.getByText(/Click a tile, then click a blank/i)).toBeInTheDocument();
  });

  it('selects a tile by click', () => {
    const onSelect = vi.fn();
    render(<TileTray tiles={TILES} onSelect={onSelect} />);
  ...
  it('gives the held tile the standard selected treatment', () => {
    render(<TileTray tiles={TILES} selectedTileId="decoy-0" onSelect={vi.fn()} />);
  ...
  it('shows a placed tile as consumed instead of dropping it from the tray', () => {
    render(<TileTray tiles={TILES} usedTileIds={['answer-5']} onSelect={vi.fn()} />);
  ...
  it('publishes the tile id on drag start and selects it, so the drop reuses the click path', () => {
    const onSelect = vi.fn();
    render(<TileTray tiles={TILES} onSelect={onSelect} />);
  ...
  it('does not toggle the held tile off when the same tile starts a drag', () => {
    const onSelect = vi.fn();
    render(<TileTray tiles={TILES} selectedTileId="answer-2" onSelect={onSelect} />);
  ...
  it('locks the whole tray once the round is graded', () => {
    render(<TileTray tiles={TILES} disabled onSelect={vi.fn()} />);
