...
import { Chip, Collapsible, Well } from "../ui";
...
export function BigOChipRow({ chips }: { chips: BigOChip[] }): ReactElement {
  return (
    <div className="flex flex-wrap gap-2">
      {chips.map((chip) => (
        <Well key={chip.label} padding="sm" className="flex flex-col gap-1 min-w-[64px]">
...
export function ComplexityCard({
  ...
  return (
    <Collapsible title="Complexity" defaultOpen className="border-[var(--border-default)]">
