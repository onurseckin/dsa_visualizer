...
export const mathAndNumberTheoryExecutions = [
  ...
  defineDsaExecution({
    ...
    cases: [
      ...cases(
        {
          ...
          expected: 0.7327777870686083,
          comparison: "float",
          tolerance: 0.0000001,
        },
        {
          label: "No stopping threshold",
          input: { n: 0, k: 0, maxPts: 10 },
          expected: 1.0,
          comparison: "float",
          tolerance: 0.0000001,
        },
        {
          label: "One draw succeeds",
      ...
      ...extraCases(
        {
          label: "Maximum reachable target",
        ...
        {
          label: "Only draw one succeeds",
          input: { n: 0, k: 1, maxPts: 10 },
          expected: 0.0,
          comparison: "float",
          tolerance: 0.0000001,
        },
        {
          label: "Two winning first draws",
          input: { n: 1, k: 1, maxPts: 10 },
          expected: 0.1,
          comparison: "float",
          tolerance: 0.0000001,
        },
        {
          label: "Two-point game reaches target",
          input: { n: 3, k: 2, maxPts: 2 },
          expected: 1.0,
          comparison: "float",
          tolerance: 0.0000001,
        },
        {
          label: "Target below every terminal score",
          input: { n: 0, k: 2, maxPts: 2 },
          expected: 0.0,
          comparison: "float",
          tolerance: 0.0000001,
        },
    ...
    audit: {
      signature: "Solution().new21Game(n: int, k: int, maxPts: int) -> float",
      defaultInputShape: "{ n: number; k: number; maxPts: number }",
      argumentMapping: ["n <- $.n", "k <- $.k", "maxPts <- $.maxPts"],
      mutation: "No input mutation.",
  ...
  defineDsaExecution({
    ...
    cases: [
      ...
      ...extraCases(
        ...
        {
          label: "Impossible head",
          input: { prob: [0.0], target: 1 },
          expected: 0.0,
          comparison: "float",
          tolerance: 0.0000001,
        },
        {
          label: "Two biased coins",
          input: { prob: [0.2, 0.8], target: 1 },
          expected: 0.68,
          comparison: "float",
          tolerance: 0.0000001,
        },
    ...
    audit: {
      signature: "Solution().probabilityOfHeads(prob: list[float], target: int) -> float",
      defaultInputShape: "{ prob: number[]; target: number }",
      argumentMapping: ["prob <- $.prob", "target <- $.target"],
      mutation: "No input mutation.",
