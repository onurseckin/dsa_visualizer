...
describe("dynamic-2d-block-prefix-sum (Block-Tiled 2D Prefix Sum Engine)", () => {
  it("should have correct metadata", () => {
    ...
    expect(dynamic2dBlockPrefixSum.topicIds.some((topicId) => topicId.startsWith("ml_"))).toBe(
      true,
    );
    expect(dynamic2dBlockPrefixSum.topicIds).toContain("ml_gemm_roofline");
    expect(dynamic2dBlockPrefixSum.topicIds).toContain("ml_gemm_roofline");
  });
  ...
  it("should map every line of code in lineExplanations", () => {
    ...
    const explanations = requireLineExplanations(dynamic2dBlockPrefixSum);
