...
export const generateConv1dSharedMemoryScratchpadSteps = (
  ...
  const getMatrixSnapshot = (
    ...
    for (let c = 0; c < maxCols; c++) {
      ...
      if (c < haloPad || c >= n + haloPad) {
        ...
        state = "active";
      } else if (phase.includes("conv") && currentI !== undefined) {
        if (currentK !== undefined && c === currentI + currentK) {
          state = "pivot";
        } else if (c >= currentI && c < currentI + kLen) {
          state = "compared";
        } else {
          state = "default";
        }
    ...
    for (let c = 0; c < maxCols; c++) {
      if (c < n) {
        ...
        cells.push({
          ...
          label: `Out[${c}]`,
          state:
            currentI === c && phase.includes("conv") ? "active" : hasVal ? "sorted" : "default",
        });
      } else {
        cells.push({
          row: 3,
    ...
    return {
      ...
      rowHeaders: ["Input Signal", "Kernel Filter", "SRAM Tile (Halo Padded)", "Conv Output"],
  ...
  // Lines 5-6: Load signal into SRAM scratchpad
  for (let i = 0; i < n; i++) {
    addStep(
      5,
  ...
  // Lines 8-12: Convolution sliding window
  for (let i = 0; i < n; i++) {
    addStep(
      8,
...
const CONV1DSHAREDMEMORYSCRATCHPAD_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 4, hint: "Allocate SRAM tile size as n + 2 * halo_pad to fit boundary halo elements." },
    {
      line: 11,
      hint: "Read window inputs directly from on-chip SRAM tile sram_tile[i + k] instead of HBM DRAM.",
    },
  ],
  lineExplanations: {
    1: "Defines conv1d_shared_memory_scratchpad function taking signal, kernel, and halo_pad parameters.",
    2: "Gets input signal length n.",
    3: "Gets kernel filter length k_len.",
    4: "Allocates on-chip SRAM scratchpad tile sized to hold signal data plus left/right halo padding.",
    5: "Iterates through each element i of input signal array.",
    6: "Copies signal[i] into sram_tile at index shifted by halo_pad offset.",
    7: "Initializes empty output array for storing convolution dot products.",
    8: "Iterates through signal sliding window output positions i from 0 to n - 1.",
    9: "Resets dot product accumulator acc to zero for output position i.",
    10: "Iterates through kernel weight filter indices k from 0 to k_len - 1.",
    11: "Executes Multiply-Accumulate (MAC): multiplies SRAM tile entry sram_tile[i + k] by kernel[k].",
    12: "Appends completed window dot product accumulator value acc to output array.",
    13: "Returns computed 1D convolution output array and sram_tile scratchpad snapshot.",
  },
};
    ...
    examples: [
      {
        ...
        outputDisplay: "Output = [-4, -4, -4, -4, 8]",
        input: DEFAULT_CONV1DSHAREDMEMORYSCRATCHPAD_INPUT,
        output: "Output = [-4, -4, -4, -4, 8]",
        explanation:
          "Loads 1D signal into SRAM scratchpad with halo slots and computes sliding window dot products.",
      },
      {
        kind: "complex",
        title: "Edge Detection Signal Filter",
        inputDisplay: "signal = [1, 1, 1, 5, 5, 5], kernel = [-1, 1], haloPad = 1",
        outputDisplay: "Output = [1, 0, 0, 4, 0, 0]",
        input: { signal: [1, 1, 1, 5, 5, 5], kernel: [-1, 1], haloPad: 1 },
        output: "Output = [1, 0, 0, 4, 0, 0]",
    ...
    topicGuide: {
      ...
      keyTerms: [
        ...
        },
        {
          ...
          definition: "Hardware execution instruction computing a * b + c in a single clock cycle.",
        },
        {
          term: "Bank Conflict",
          definition:
