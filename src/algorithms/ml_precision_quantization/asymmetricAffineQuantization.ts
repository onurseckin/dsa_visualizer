...
export const ASYMMETRICAFFINEQUANTIZATION_CODE = "def asymmetric_affine_quantization(input_data: list) -> list:\n    # Full Affine INT8 Quantize & Dequantize Pipeline (Medium)\n    # Implements full bidirection affine quantization Q = clamp(round(X/S) + Z).\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const generateAsymmetricAffineQuantizationSteps = (
  ...
  addStep(
    1,
    "Initialize Full Affine INT8 Quantize & Dequantize Pipeline",
...
const ASYMMETRICAFFINEQUANTIZATION_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for Full Affine INT8 Quantize & Dequantize Pipeline.",
...
export const asymmetricAffineQuantization: AlgorithmDefinition<asymmetricAffineQuantizationInput> = {
  ...
  topicGuide: {
    overview: "Affine quantization maps arbitrary float intervals [min, max] to [-128, 127].",
    sections: [
      { heading: "Core Concept", body: "Implements full bidirection affine quantization Q = clamp(round(X/S) + Z)." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"Affine Quantization","definition":"Linear scale and offset mapping to integer ranges."}],
  },
