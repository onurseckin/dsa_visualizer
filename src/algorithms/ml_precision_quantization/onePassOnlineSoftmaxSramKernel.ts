...
export const ONEPASSONLINESOFTMAXSRAMKERNEL_CODE = "def one_pass_online_softmax_sram_kernel(input_data: list) -> list:\n    # 1-Pass Online Softmax Streaming GPU Kernel (Hard)\n    # Computes Softmax block-by-block in a single pass without storing full matrices.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const generateOnePassOnlineSoftmaxSramKernelSteps = (
  ...
  addStep(
    1,
    "Initialize 1-Pass Online Softmax Streaming GPU Kernel",
...
const ONEPASSONLINESOFTMAXSRAMKERNEL_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for 1-Pass Online Softmax Streaming GPU Kernel.",
...
export const onePassOnlineSoftmaxSramKernel: AlgorithmDefinition<onePassOnlineSoftmaxSramKernelInput> = {
  ...
  topicGuide: {
    overview: "Online Softmax updates running max m_new and sum d_new across streaming tiles.",
    sections: [
      { heading: "Core Concept", body: "Computes Softmax block-by-block in a single pass without storing full matrices." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"Online Softmax","definition":"Streaming Softmax normalization in 1 pass."}],
  },
