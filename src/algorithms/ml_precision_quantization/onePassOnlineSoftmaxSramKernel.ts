...
export const generateOnePassOnlineSoftmaxSramKernelSteps = (
  ...
  logits.forEach((x, idx) => {
    ...
    addStep(
      ...
      { idx, x, dMax: dMax === Number.NEGATIVE_INFINITY ? "-inf" : Number(dMax.toFixed(4)), isNewMax, phase: "CHECK_MAX" },
      x,
      dSum,
    );
    ...
    if (isNewMax) {
      ...
      if (prevMax !== Number.NEGATIVE_INFINITY) {
        ...
        addStep(
          ...
          { prevMax: Number(prevMax.toFixed(4)), x, rescaleFactor: Number(rescaleFactor.toFixed(6)), dSum: Number(dSum.toFixed(6)), phase: "RESCALE_SUM" },
          x,
          dSum,
        );
      } else {
    ...
    addStep(
      ...
      { idx, x, dMax: Number(dMax.toFixed(4)), expTerm: Number(expTerm.toFixed(6)), dSum: Number(dSum.toFixed(6)), phase: "ACCUMULATE_EXP" },
      x,
      dSum,
    );
  });
...
const ONEPASSONLINESOFTMAXSRAMKERNEL_TRIVIA: TriviaMeta = {
  ...
  hints: [
    ...
    { line: 7, hint: "Rescale running sum d_sum by exp(d_max_old - x) when a new maximum is found." },
...
- **Trade-Off**: Eliminates HBM DRAM memory bandwidth bottlenecks by fusing Softmax reduction inside GPU SRAM tiles.`,
  ...
  topicGuide: {
    ...
    keyTerms: [
      {
        term: "FlashAttention",
        definition: "IO-aware attention algorithm fusing matmul and Softmax inside GPU SRAM memory.",
      },
      {
        term: "Online Softmax",
