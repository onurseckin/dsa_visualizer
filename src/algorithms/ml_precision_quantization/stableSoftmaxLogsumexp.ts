...
export const STABLESOFTMAXLOGSUMEXP_CODE = "def stable_softmax_logsumexp(input_data: list) -> list:\n    # Numerically Stable Softmax & LogSumExp (Medium)\n    # Computes LogSumExp LSE(x) = max(x) + log sum(exp(x - max(x))).\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const generateStableSoftmaxLogsumexpSteps = (
  ...
  addStep(
    1,
    "Initialize Numerically Stable Softmax & LogSumExp",
...
const STABLESOFTMAXLOGSUMEXP_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for Numerically Stable Softmax & LogSumExp.",
...
export const stableSoftmaxLogsumexp: AlgorithmDefinition<stableSoftmaxLogsumexpInput> = {
  ...
  topicGuide: {
    overview: "LogSumExp trick computes log-domain sums without numeric overflow.",
    sections: [
      { heading: "Core Concept", body: "Computes LogSumExp LSE(x) = max(x) + log sum(exp(x - max(x)))." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"LogSumExp","definition":"Numerically stable sum of exponentials."}],
  },
