...
export const LOGICALTOPHYSICALBLOCKADDRESSTRANSLATOR_CODE = "def logical_to_physical_block_address_translator(input_data: list) -> list:\n    # PagedAttention Logical to Physical Address Translator (Easy)\n    # Translates logical token index t into physical address Addr(t) = BlockPtr * B + (t % B).\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const logicalToPhysicalBlockAddressTranslator: AlgorithmDefinition<logicalToPhysicalBlockAddressTranslatorInput> = {
  ...
  topicGuide: {
    overview: "Logical token t maps to physical block table entry via floor(t / B) and offset t % B.",
    sections: [
      { heading: "Core Concept", body: "Translates logical token index t into physical address Addr(t) = BlockPtr * B + (t % B)." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"Address Translation","definition":"Mapping logical token index to physical GPU VRAM address."}],
  },
