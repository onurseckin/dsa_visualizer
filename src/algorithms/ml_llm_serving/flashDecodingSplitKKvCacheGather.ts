...
export const FLASHDECODINGSPLITKKVCACHEGATHER_CODE = "def flash_decoding_split_k_kv_cache_gather(input_data: list) -> list:\n    # FlashDecoding Split-K KV Cache Gather Engine (Hard)\n    # Gathers split-K KV cache sequence partitions across GPU thread blocks.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
const FLASHDECODINGSPLITKKVCACHEGATHER_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for FlashDecoding Split-K KV Cache Gather Engine.",
...
export const flashDecodingSplitKKvCacheGather: AlgorithmDefinition<flashDecodingSplitKKvCacheGatherInput> = {
  ...
  topicGuide: {
    overview: "FlashDecoding split-K gathers partial softmax outputs across sequence partition blocks.",
    sections: [
      { heading: "Core Concept", body: "Gathers split-K KV cache sequence partitions across GPU thread blocks." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"FlashDecoding Gather","definition":"Combining split-K partial attention outputs."}],
  },
