...
export const FULLSPECULATIVEDECODINGSERVINGENGINE_CODE = "def full_speculative_decoding_serving_engine(input_data: list) -> list:\n    # Full Speculative Decoding Production Serving Engine (Hard)\n    # Orchestrates draft model generation, target verification, rejection sampling, and KV rollback.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const fullSpeculativeDecodingServingEngine: AlgorithmDefinition<fullSpeculativeDecodingServingEngineInput> = {
  ...
  topicGuide: {
    ...
    sections: [
      { heading: "Core Concept", body: "Orchestrates draft model generation, target verification, rejection sampling, and KV rollback." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"Speculative Engine","definition":"End-to-end speculative decoding inference server."}],
  },
