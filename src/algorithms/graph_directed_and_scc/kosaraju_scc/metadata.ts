...
export const KOSARAJU_SCC_TRIVIA: TriviaMeta = {
  lineExplanations: {
    ...
    2: "Visited set for Pass 1 to prevent visiting nodes more than once.",
    3: "Finish-time stack: nodes are pushed when all their outgoing edges are done.",
    4: "Pass 1 DFS helper for a single vertex.",
    5: "Marks u visited in Pass 1.",
    6: "Iterates through outgoing neighbors of u.",
    7: "Recurses on unvisited neighbor v.",
    8: "Recurses first, so u can only finish after everything reachable from it has already finished.",
    9: "Pushes u onto the stack the moment it finishes — exactly the post-order that puts upstream vertices on top.",
    11: "Sweeps every vertex to make sure disconnected pieces of the graph get their own DFS too.",
    12: "Only launches a fresh DFS from vertices no prior call has already reached.",
    13: "Starts pass 1 from this unvisited vertex, extending the finish-order stack to cover its whole reachable region.",
    15: "Resets visited tracking for pass 2 — reusing pass 1's set here would make the second sweep do nothing, since everything would already look visited.",
    16: "Will collect each discovered strongly connected component as its own list of vertices.",
    17: "A depth-first search on the transposed (edge-reversed) graph that collects every vertex it reaches into one component.",
    18: "Marks u claimed by the component currently being built, using pass 2's own visited set.",
    19: "Adds u to the SCC currently under construction.",
    20: "Walks u's neighbors in the reversed graph — reachability here corresponds to backward reachability in the original graph.",
    21: "Skips any vertex already claimed by this or an earlier component.",
    22: "Recurses to pull v into the same component; the reversed edges keep this search fenced inside one true SCC.",
    24: "Processes vertices in reverse finish order — last-finished first — which is what guarantees each restart lands on an unclaimed component root.",
    25: "Pops the top of the finish stack, the latest-finishing vertex still remaining.",
    26: "Only starts a new component search if u hasn't already been swept into an earlier one.",
    27: "Opens a fresh, empty component list to be filled by this round of DFS.",
    28: "Runs the fenced search on the transpose, collecting exactly one full strongly connected component starting from u.",
    29: "Records the completed component before moving on to the next unclaimed vertex on the stack.",
    30: "Every vertex has now been assigned to exactly one component — the full SCC decomposition of the graph.",
  },
};
