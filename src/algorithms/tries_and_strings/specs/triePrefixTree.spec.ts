...
import { DEFAULT_TRIE_INPUT, generateTriePrefixTreeSteps, triePrefixTree } from "../triePrefixTree";
...
describe("triePrefixTree algorithm spec", () => {
  it("should have valid definition metadata", () => {
    ...
    expect(triePrefixTree.topicIds).toContain("tries_and_strings");
  ...
  it("should distinguish search for prefix-only word from complete word and handle non-existent prefix", () => {
    const input = {
      wordsToInsert: ["cat", "car"],
      searchWord: "ca", // 'ca' exists in trie but is not end-of-word
      prefixToSearch: "z", // prefix 'z' does not exist
    };
    const steps = generateTriePrefixTreeSteps(input);

    const searchStep = steps.find(
      (s) =>
        s.variables.operation === "search" &&
        s.variables.targetWord === "ca" &&
        s.variables.found === false,
    );
    expect(searchStep).toBeDefined();

    const prefixStep = steps.find(
      (s) =>
        s.variables.operation === "startsWith" &&
        s.variables.prefix === "z" &&
        s.variables.found === false,
    );
