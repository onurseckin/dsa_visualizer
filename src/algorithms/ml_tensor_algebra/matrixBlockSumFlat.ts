...
export const generateMatrixBlockSumFlatSteps = (
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (mode === "prefix") {
          ...
          else if (r < (activeR ?? 0) || (r === activeR && c < (activeC ?? 0))) state = "sorted";
        } else if (mode === "result") {
          cellVal = result[r][c];
  ...
  const addStep = (
    ...
    steps.push({
      ...
      primarySnapshot: makeMatrixSnapshot(mode, activeR, activeC, windowBounds, titleText),
      auxiliaryState: {
        customState: {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      addStep(
        ...
        `Applying 2D recurrence: P[r+1][c+1] = matrix[r][c] + P[r][c+1] + P[r+1][c] - P[r][c].`,
...
const MATRIXBLOCKSUMFLAT_TRIVIA: TriviaMeta = {
  ...
  hints: [
    {
      line: 15,
      hint: "Use the 4-corner prefix formula: P[r2+1][c2+1] - P[r1][c2+1] - P[r2+1][c1] + P[r1][c1].",
    },
  ],
  lineExplanations: {
    1: "Function declaration taking input matrix grid and radius k.",
    2: "Retrieves row count of input matrix.",
    3: "Retrieves column count of input matrix.",
    4: "Allocates zero-initialized prefix grid of size (rows + 1) x (cols + 1).",
    5: "Empty line separating initialization from prefix calculation loops.",
    6: "Outer loop iterating through row indices r of input matrix.",
    7: "Inner loop iterating through column indices c of input matrix.",
    8: "Computes prefix integral cell using recurrence: matrix[r][c] + top + left - top_left.",
    9: "Empty line separating prefix build loop from result query loop.",
    10: "Allocates output result matrix of size rows x cols initialized to zeros.",
    11: "Outer loop iterating through result matrix row indices r.",
    12: "Inner loop iterating through result matrix column indices c.",
    13: "Calculates top-left boundary coordinates [r1, c1] clamped to matrix limits.",
    14: "Calculates bottom-right boundary coordinates [r2, c2] clamped to matrix limits.",
    15: "Queries 2D prefix sum table using 4-corner inclusion-exclusion equation.",
    16: "Empty line separating query loops from return statement.",
    17: "Returns completed 2D matrix containing k-radius submatrix sums.",
  },
};
...
export const matrixBlockSumFlat: AlgorithmDefinition<matrixBlockSumFlatInput> = {
  ...
  examples: [
    ...
    },
    {
      ...
      output: "[[4, 4], [4, 4]]",
      explanation:
        "When k exceeds grid dimensions, every cell receives the sum of the entire matrix.",
    },
    {
      kind: "negative",
    ...
    keyTerms: [
      {
        ...
        definition:
          "A 2D array representation where each cell stores the sum of all grid values above and to its left.",
      },
      {
        term: "Inclusion-Exclusion Principle",
        definition:
          "Mathematical technique to compute union area by summing component areas and subtracting overlapping intersections.",
      },
      {
        term: "Integral Image",
