...
describe("ast-expression-eval-variables (AST Expression Evaluation with Variables)", () => {
  it("should have correct metadata", () => {
    ...
    expect(astExpressionEvalVariables.topicIds.some((topicId) => topicId.startsWith("ml_"))).toBe(
      true,
    );
    expect(astExpressionEvalVariables.topicIds).toContain("ml_autograd_dags");
    expect(astExpressionEvalVariables.topicIds).toContain("ml_autograd_dags");
  });
