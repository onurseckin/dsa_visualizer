...
export const FORD_FULKERSON_TRIVIA: TriviaMeta = {
  lineExplanations: {
    ...
    2: "Comment explaining capacity and flow initialization.",
    3: "Initializes empty capacity table mapping edge tuples to max limits.",
    4: "Initializes empty flow table mapping edge tuples to current flow.",
    5: "Iterates through each input edge tuple to populate initial values.",
    6: "Sets initial capacity limit for directed edge (u, v).",
    7: "Initializes starting flow to zero for directed edge (u, v).",
    9: "Defines helper recursive depth-first search to find an augmenting path.",
    10: "Base case: reached the sink target node, return the bottleneck flow found along this path.",
    ...
    13: "Filters for outgoing edges from current node u to an unvisited neighbor v_node.",
    14: "Calculates remaining residual capacity: max capacity minus current flow.",
    15: "Only proceed if there is positive residual capacity available.",
    16: "Recursively search deeper from v_node, updating minimum bottleneck capacity along path.",
    17: "If a valid augmenting path to sink was found with positive flow:",
    18: "Return the bottleneck flow amount back up the recursion stack.",
    ...
    22: "Enters main loop to repeatedly find and augment paths until none remain.",
