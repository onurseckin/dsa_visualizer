...
export const generateSingleFeatureSplitSteps = (
  ...
  const featureValues = input.featureValues || input.data || [1.2, 1.8, 2.5, 2.9, 3.1, 3.8, 4.2, 4.8, 5.0, 5.5];
...
const SINGLE_FEATURE_THRESHOLD_SPLIT_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 10, hint: "Test if feature value is less than or equal to continuous threshold t." },
    { line: 11, hint: "Append index to left_indices for left branch (x <= t)." },
    { line: 13, hint: "Append index to right_indices for right branch (x > t)." },
  ],
  lineExplanations: {
    1: "Defines entry point for Single Feature Threshold Splitter function.",
    2: "Docstring opening delimiter tag.",
    3: "Describes continuous feature threshold partitioning X[j] <= threshold.",
    4: "Docstring detailing tuple return signature (leftIndices, rightIndices).",
    5: "Docstring closing delimiter tag.",
    6: "Initializes empty list left_indices for left child node sample indices.",
    7: "Initializes empty list right_indices for right child node sample indices.",
    8: "Blank line before sample iteration loop.",
    9: "Iterates over feature values array yielding sample index idx and scalar value val.",
    10: "Evaluates split threshold condition: is feature val <= threshold?",
    11: "Appends sample index idx to left_indices for left branch.",
    12: "Else branch executed when feature val > threshold.",
    13: "Appends sample index idx to right_indices for right branch.",
    14: "Blank line separating partition loop from return statement.",
    15: "Returns tuple of (left_indices, right_indices) sample index lists.",
  },
};
...
export const singleFeatureThresholdSplit: AlgorithmDefinition<SingleFeatureThresholdSplitInput> = {
  ...
  constraints: [
    "1 <= N <= 1000000",
    "feature_values elements are finite floats",
  ],
  examples: [
    {
      kind: "basic",
      title: "10-Sample Feature Split at Threshold t=3.0",
      inputDisplay: "featureValues = [1.2, 1.8, 2.5, 2.9, 3.1, 3.8, 4.2, 4.8, 5.0, 5.5], threshold = 3.0",
    ...
    keyTerms: [
      {
        ...
        definition: "Binary partition predicate X_j <= t dividing sample indices into left and right child nodes.",
      },
      {
        term: "Axis-Aligned Hyperplane",
        definition: "Decision surface orthogonal to a single feature axis in multi-dimensional space.",
      },
      {
        term: "Index Routing",
        definition: "Storing 32-bit sample index references in child lists instead of copying full feature rows.",
      },
      {
