...
export const XGBOOSTSPLITGAINSCORECALCULATOR_CODE = "def xgboost_split_gain_score_calculator(input_data: list) -> list:\n    # XGBoost 2nd-Order Split Gain Score Calculator (Medium)\n    # Evaluates split gain score L_split = 0.5 * [ G_L^2/(H_L+lambda) + G_R^2/(H_R+lambda) - G^2/(H+lambda) ] - gamma.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const generateXgboostSplitGainScoreCalculatorSteps = (
  ...
  addStep(
    1,
    "Initialize XGBoost 2nd-Order Split Gain Score Calculator",
...
const XGBOOSTSPLITGAINSCORECALCULATOR_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for XGBoost 2nd-Order Split Gain Score Calculator.",
...
export const xgboostSplitGainScoreCalculator: AlgorithmDefinition<xgboostSplitGainScoreCalculatorInput> = {
  ...
  topicGuide: {
    overview: "Split gain score measures loss reduction accounting for tree complexity penalty gamma.",
    sections: [
      { heading: "Core Concept", body: "Evaluates split gain score L_split = 0.5 * [ G_L^2/(H_L+lambda) + G_R^2/(H_R+lambda) - G^2/(H+lambda) ] - gamma." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"Split Gain Score","definition":"XGBoost loss reduction gain formula."}],
  },
