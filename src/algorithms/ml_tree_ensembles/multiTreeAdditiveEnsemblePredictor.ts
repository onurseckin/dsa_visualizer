...
export const generateMultiTreeAdditiveSteps = (
  ...
  const treeLeafValues = input.treeLeafValues ||
    input.data || [1.5, -0.8, 0.4, 0.2, -0.5, 0.6, 0.3, -0.4, 0.5, 0.1, -0.2, 0.4];
  let stepIndex = 0;
  ...
  treeLeafValues.forEach((leafVal, k) => {
    ...
    steps.push({
      ...
      auxiliaryState: {
        customState: { "Cumulative Margins": `[${cumulativeMargins.join(", ")}]` },
      },
...
const MULTI_TREE_ADDITIVE_PREDICTOR_TRIVIA: TriviaMeta = {
  ...
  hints: [
    {
      line: 11,
      hint: "GBDT additive expansion equation: curr_margin += learning_rate * leaf_val.",
    },
    { line: 14, hint: "Final classification probability: sigmoid(curr_margin)." },
  ],
  lineExplanations: {
    ...
    7: "Initializes empty list cumulative_margins to record step-by-step margin progress.",
    8: "Initializes curr_margin = base_score prior value.",
    9: "Blank line before tree boosting loop.",
    10: "Iterates over tree index k and leaf output weight leaf_val in enumerate(tree_leaf_values).",
    11: "Accumulates shrunk tree prediction into margin: curr_margin += learning_rate * leaf_val.",
    12: "Appends rounded curr_margin to cumulative_margins list.",
    13: "Blank line separating boosting loop from final probability transform.",
    14: "Transforms final raw margin to classification probability final_prob = round(sigmoid(curr_margin), 4).",
    15: "Returns tuple of (curr_margin, final_prob, cumulative_margins).",
  },
};
    ...
    examples: [
      {
        ...
        explanation:
          "Accumulates 12 shrunk tree outputs (eta=0.1) onto base score 0.0, yielding final margin 0.25 and probability 0.5622.",
      },
    ],
    ...
    topicGuide: {
      ...
      keyTerms: [
        ...
        },
        {
          ...
          definition:
            "Probability transform 1 / (1 + exp(-z)) converting margin z into probability [0, 1].",
        },
      ],
    },
