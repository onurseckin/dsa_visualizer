...
export const generateShannonEntropySteps = (
  ...
  steps.push({
    ...
    explanation: {
      ...
      why:
        n === 0
          ? "Labels list is empty. Return 0.0."
          : `Labels list has length N = ${n} (not empty). Proceed to frequency counting.`,
    },
    primarySnapshot: {
      kind: "array",
      elements: labels.map((y, idx) => ({
    ...
    auxiliaryState: { customState: { "Empty Check": n === 0 ? "True" : "False" } },
    variables: { is_empty: n === 0 },
  });
  ...
  if (n === 0) {
    steps.push({
      ...
      codeLine: 5,
      explanation: {
        what: "Return 0.0 for Empty Dataset",
        why: "An empty dataset has no uncertainty; entropy is 0.0.",
      },
      primarySnapshot: {
        kind: "array",
        elements: [],
      },
      auxiliaryState: { customState: { "Shannon Entropy H(S)": "0.0 bits" } },
      variables: { entropy: 0.0, completed: true },
    });
    return steps;
  }
    ...
    steps.push({
      ...
      primarySnapshot: {
        ...
        elements: labels.map((y, idx) => ({
          ...
          label: `y=${y}`,
          state:
            String(y) === classLabel ? ("active" as ElementState) : ("visited" as ElementState),
        })),
      },
    ...
    steps.push({
      ...
      primarySnapshot: {
        ...
        elements: labels.map((y, idx) => ({
          ...
          label: `y=${y}`,
          state:
            String(y) === classLabel ? ("compare" as ElementState) : ("visited" as ElementState),
        })),
      },
    ...
    steps.push({
      ...
      primarySnapshot: {
        ...
        elements: labels.map((y, idx) => ({
          ...
          label: `y=${y}`,
          state:
            String(y) === classLabel ? ("sorted" as ElementState) : ("visited" as ElementState),
        })),
      },
...
const SHANNON_ENTROPY_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 13, hint: "Iterate over frequency counts of each unique class in the dataset." },
    { line: 16, hint: "Shannon Entropy term accumulation: entropy -= p * math.log2(p)." },
  ],
  lineExplanations: {
    ...
    4: "Checks if labels list is empty.",
    5: "Returns 0.0 entropy for empty label list.",
    6: "Blank line before sample size extraction.",
    7: "Measures total sample count n in labels list.",
    8: "Initializes empty dictionary counts to accumulate class frequencies.",
    9: "Iterates over class label y in labels list.",
    10: "Increments frequency counter counts[y] for current class y.",
    11: "Blank line before entropy accumulation loop.",
    12: "Initializes floating point accumulator entropy = 0.0.",
    13: "Iterates over sample frequency counts cnt in counts.values().",
    14: "Calculates class probability p = cnt / n.",
    15: "Checks if probability p is positive (p > 0).",
    16: "Subtracts p * math.log2(p) from entropy accumulator.",
    17: "Blank line separating accumulation loop from return statement.",
    18: "Rounds accumulated entropy to 4 decimal places and returns final float value.",
  },
};
...
export const shannonEntropyCalculator: AlgorithmDefinition<ShannonEntropyCalculatorInput> = {
  ...
  constraints: ["1 <= N <= 1000000", "Class labels are non-negative integers"],
  examples: [
    {
      kind: "basic",
      ...
      output: "1.5546",
      explanation:
        "Evaluates H(S) = -(4/12 log2(4/12) + 5/12 log2(5/12) + 3/12 log2(3/12)) = 1.5546 bits.",
    },
  ],
  ...
  topicGuide: {
    ...
    keyTerms: [
      ...
      },
      {
        ...
        definition:
          "Reduction in entropy achieved by partitioning a dataset: H(Parent) - Weighted_Child_Entropy.",
      },
      {
        term: "ID3 Algorithm",
        definition:
          "Iterative Dichotomiser 3 decision tree algorithm developed by Ross Quinlan using Information Gain.",
      },
      {
