...
export const generateOptimalLeafWeightSteps = (
  ...
  const gradients = input.gradients ||
    input.data || [-0.8, -0.6, -0.4, -0.2, 0.1, 0.3, 0.5, 0.7, 0.9, -0.3];
  ...
  // Step 2: Sum gradients (codeLine 2)
  let G = 0.0;
  ...
  // Step 3: Sum hessians (codeLine 3)
  let H = 0.0;
  hessians.forEach((h, idx) => {
    ...
    steps.push({
      ...
      primarySnapshot: {
        kind: "array",
        elements: gradients.map((_, i) => ({
          id: `s-${i}`,
          value: Number(hessians[i].toFixed(2)),
          label: `h=${hessians[i]}`,
          state:
            i === idx
              ? ("active" as ElementState)
      ...
      auxiliaryState: {
        customState: { "Gradient Sum G": G.toFixed(4), "Hessian Sum H": H.toFixed(4) },
      },
      variables: { idx, h, H },
    });
  });

  // Step 4: Compute w_star (codeLine 4)
...
const REGULARIZED_OPTIMAL_LEAF_WEIGHT_TRIVIA: TriviaMeta = {
  ...
  hints: [
    {
      line: 4,
      hint: "Optimal regularized leaf weight formula in XGBoost: w* = - G / (H + lambda).",
    },
  ],
  lineExplanations: {
    1: "Defines entry point for compute_regularized_optimal_leaf_weight function.",
    2: "Sums 1st-order gradients G = sum(gradients) across all samples in leaf node.",
    3: "Sums 2nd-order hessians H = sum(hessians) across all samples in leaf node.",
    4: "Calculates optimal regularized leaf weight w_star = - G / (H + lambda_reg).",
    5: "Returns tuple of (rounded w_star, rounded G, rounded H).",
  },
};
...
export const regularizedOptimalLeafWeight: AlgorithmDefinition<RegularizedOptimalLeafWeightInput> =
  {
    ...
    constraints: ["1 <= N <= 1000000", "lambdaReg >= 0.0", "Hessian sum H > 0"],
    examples: [
      {
        kind: "basic",
      ...
      keyTerms: [
        {
          ...
          definition:
            "Analytical leaf prediction output -G / (H + lambda) minimizing 2nd-order Taylor loss objective.",
        },
        {
          term: "L2 Regularization (Lambda)",
          definition:
            "Penalty term lambda shrink-controlling leaf weight magnitude to prevent overfitting.",
        },
        {
