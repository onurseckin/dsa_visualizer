...
export const FP16MODELMEMORYFOOTPRINTCALCULATOR_CODE = `def fp16_model_memory_footprint_calculator(layer_param_counts: list[int], optimizer_type: str = "adam") -> dict:
    ...
    if optimizer_type.lower() == "adam":
        optimizer_states_bytes = total_params * 8
    else:
        optimizer_states_bytes = total_params * 4
...
const FP16MODELMEMORYFOOTPRINTCALCULATOR_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    ...
    2: "Sums parameter counts across all model layers to find total_params.",
    3: "Validates positive total parameter count.",
    4: "Returns zero memory dictionary if total_params <= 0.",
    5: "Blank line before memory calculations.",
    6: "Computes 2 bytes per parameter for FP16 model weights.",
    7: "Computes 2 bytes per parameter for FP16 gradients.",
    8: "Computes 4 bytes per parameter for FP32 master weights.",
    9: "Blank line before optimizer condition.",
    10: "Checks if optimizer is Adam.",
    11: "Assigns 8 bytes per parameter for Adam optimizer states (4B momentum + 4B variance).",
    12: "Else block for non-Adam optimizers.",
    13: "Assigns 4 bytes per parameter for momentum optimizers.",
    14: "Blank line before summing total static bytes.",
    15: "Starts expression for total static memory bytes summation.",
    16: "Sums FP16 params, FP16 grads, FP32 master params, and optimizer states.",
    17: "Closes total_static_bytes summation expression.",
    18: "Blank line before return dictionary construction.",
    19: "Starts dictionary construction for memory breakdown results.",
    20: "Includes total_params key.",
    21: "Converts FP16 parameters bytes to Megabytes.",
    22: "Converts FP16 gradients bytes to Megabytes.",
    23: "Converts FP32 master parameters bytes to Megabytes.",
    24: "Converts optimizer states bytes to Megabytes.",
    25: "Converts total static bytes to Gigabytes (GB).",
    26: "Calculates bytes_per_parameter integer ratio.",
    27: "Closes return dictionary construct.",
  },
};
...
export const fp16ModelMemoryFootprintCalculator: AlgorithmDefinition<fp16ModelMemoryFootprintCalculatorInput> =
  {
    ...
    topicIds: ["ml_distributed_systems", "ml_precision_quantization"],
    difficulty: "Easy",
    description:
