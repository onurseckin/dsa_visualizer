...
export const generateTILEINDEXGRIDMAPPERSteps = (
  ...
  const getSnapshot = (activePid1d: number = -1) => {
...
const TILEINDEXGRIDMAPPER_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 2, hint: "Row block index calculation: pid_1d // num_pid_n." },
    { line: 3, hint: "Column block index calculation: pid_1d % num_pid_n." },
    {
      line: 8,
      hint: "Linear pointer offset formula: start_row * stride_m + start_col * stride_n.",
    },
  ],
  lineExplanations: {
    1: "Defines entry point for map_1d_program_id_to_2d_tile function.",
    2: "Calculates 2D row block coordinate pid_m = pid_1d // num_pid_n.",
    3: "Calculates 2D column block coordinate pid_n = pid_1d % num_pid_n.",
    4: "Blank line before row/col pixel offset calculation.",
    5: "Calculates top-left row offset start_row = pid_m * block_m.",
    6: "Calculates top-left column offset start_col = pid_n * block_n.",
    7: "Blank line before global memory pointer offset calculation.",
    8: "Calculates linear HBM DRAM pointer offset global_ptr_offset = start_row * stride_m + start_col * stride_n.",
    9: "Blank line separating logic from return statement.",
    10: "Returns tuple of (pid_m, pid_n, start_row, start_col, global_ptr_offset).",
  },
};
...
export const tileIndexGridMapper: AlgorithmDefinition<tileIndexGridMapperInput> = {
  ...
  constraints: ["0 <= pid_1d <= 1000000", "numPidM >= 1", "numPidN >= 1"],
  examples: [
    {
      kind: "basic",
      ...
      explanation:
        "Decodes 8 1D program IDs into 2x4 2D matrix tile coordinates and calculates global DRAM memory offsets.",
    },
  ],
  ...
  topicGuide: {
    ...
    keyTerms: [
      ...
      },
      {
        term: "Matrix Stride",
        definition:
          "Number of elements in memory between adjacent rows (stride_m) or columns (stride_n).",
      },
      {
        term: "Global Pointer Offset",
        definition:
          "Linear memory address offset in HBM DRAM calculated as start_row * stride_m + start_col * stride_n.",
      },
    ],
  },
