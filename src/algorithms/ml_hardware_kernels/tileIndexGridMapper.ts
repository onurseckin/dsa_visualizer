...
export const TILEINDEXGRIDMAPPER_CODE = "def tile_index_grid_mapper(input_data: list) -> list:\n    # Triton SPMD Block Tile Grid Mapper (Easy)\n    # Maps 1D program ID to 2D block tile indices (pid_m, pid_n).\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const generateTileIndexGridMapperSteps = (
  ...
  addStep(
    1,
    "Initialize Triton SPMD Block Tile Grid Mapper",
...
const TILEINDEXGRIDMAPPER_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for Triton SPMD Block Tile Grid Mapper.",
...
export const tileIndexGridMapper: AlgorithmDefinition<tileIndexGridMapperInput> = {
  ...
  topicGuide: {
    ...
    sections: [
      { heading: "Core Concept", body: "Maps 1D program ID to 2D block tile indices (pid_m, pid_n)." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"SPMD Grid Map","definition":"Mapping 1D hardware thread block IDs to 2D tile coordinates."}],
  },
