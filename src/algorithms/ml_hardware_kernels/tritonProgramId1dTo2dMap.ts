...
export const TRITONPROGRAMID1DTO2DMAP_CODE = "def triton_program_id1d_to2d_map(input_data: list) -> list:\n    # Triton `tl.program_id` 1D-to-2D Coordinate Mapper (Easy)\n    # Converts linear thread block ID into (pid_m, pid_n) tile coordinates.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
const TRITONPROGRAMID1DTO2DMAP_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for Triton `tl.program_id` 1D-to-2D Coordinate Mapper.",
...
export const tritonProgramId1dTo2dMap: AlgorithmDefinition<tritonProgramId1dTo2dMapInput> = {
  ...
  topicGuide: {
    ...
    sections: [
      { heading: "Core Concept", body: "Converts linear thread block ID into (pid_m, pid_n) tile coordinates." },
