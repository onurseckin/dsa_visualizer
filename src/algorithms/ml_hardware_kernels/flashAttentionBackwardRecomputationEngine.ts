...
const FLASHATTENTIONBACKWARDRECOMPUTATIONENGINE_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 10, hint: "Precomputed softmax backward term: D_i = sum(dO_i * O_i)." },
    { line: 21, hint: "Score gradient formula: ds_ij = p_ij * (dp_ij - D_i)." },
  ],
  lineExplanations: {
    1: "Defines entry point for flash_attention_backward_recompute function.",
    2: "Docstring describing FlashAttention backward pass with on-the-fly attention score recomputation.",
    3: "Measures sequence length N = len(Q).",
    4: "Measures head dimension d = len(Q[0]).",
    5: "Blank line before allocating gradient buffers.",
    6: "Allocates Query gradient matrix dQ (N x d) filled with zeros.",
    7: "Allocates Key gradient matrix dK (N x d) filled with zeros.",
    8: "Allocates Value gradient matrix dV (N x d) filled with zeros.",
    9: "Blank line before precomputing D vector.",
    10: "Precomputes softmax backward constant vector D_i = sum(dO_i * O_i) across sequence length N.",
    11: "Blank line before query loop.",
    12: "Iterates over query token row index i from 0 to N - 1.",
    13: "Iterates over key/value token column index j from 0 to N - 1.",
    14: "Recomputes unnormalized attention score s_ij = Q[i] * K[j]^T * scale on-the-fly from Q and K.",
    15: "Recomputes softmax probability p_ij = exp(s_ij - L[i]) using stored logsumexp L[i].",
    16: "Blank line before Value gradient accumulation loop.",
    17: "Iterates over head dimension column col from 0 to d - 1.",
    18: "Accumulates Value gradient dV[j][col] += p_ij * dO[i][col].",
    19: "Blank line before score gradient computation.",
    20: "Calculates probability gradient dp_ij = sum(dO_i * V_j).",
    21: "Calculates unnormalized score gradient ds_ij = p_ij * (dp_ij - D[i]).",
    22: "Blank line before Query & Key gradient accumulation loop.",
    23: "Iterates over head dimension column col from 0 to d - 1.",
    24: "Accumulates Query gradient dQ[i][col] += scale * ds_ij * K[j][col].",
    25: "Accumulates Key gradient dK[j][col] += scale * ds_ij * Q[i][col].",
    26: "Blank line separating loops from return statement.",
    27: "Returns tuple of (dQ, dK, dV) gradient matrices.",
  },
};
...
export const flashAttentionBackwardRecomputationEngine: AlgorithmDefinition<flashAttentionBackwardRecomputationEngineInput> =
  {
    id: "flashAttentionBackwardRecomputationEngine",
    ...
    constraints: [
      "1 <= N <= 8192",
      "1 <= d <= 128",
      "Q, K, V, O, dO dimensions must match N x d",
    ],
    examples: [
      {
        kind: "basic",
        ...
        output: "([dQ], [dK], [dV])",
        explanation: "Recomputes attention scores s_ij on-the-fly from Q, K, and L, evaluating exact dQ, dK, dV gradients without saving N x N activations.",
      },
    ],
    ...
    complexityAnalysis: {
      ...
      space: "Requires $O(N \\cdot d)$ memory space for output gradients $dQ, dK, dV$, eliminating $O(N^2)$ activation memory.",
    },
    topicGuide: {
      overview:
      ...
      keyTerms: [
        {
          ...
          definition: "Re-evaluating forward activations on-the-fly during backpropagation to save VRAM memory.",
        },
        {
          term: "Logsumexp Vector (L_i)",
          definition: "O(N) vector stored during forward pass containing row softmax normalization denominators.",
        },
        {
          term: "Softmax Constant (D_i)",
          definition: "Precomputed row dot product D_i = sum(dO_i * O_i) used in softmax backpropagation.",
        },
        {
          term: "Activation Memory Saving",
          definition: "Reducing peak GPU memory footprint by discarding intermediate attention matrices.",
        },
      ],
    },
