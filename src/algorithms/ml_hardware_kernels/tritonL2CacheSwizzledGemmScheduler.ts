...
export const generateTRITONL2CACHESWIZZLEDGEMMSCHEDULERSteps = (
  ...
  const getSnapshot = (activePidM: number = -1, activePidN: number = -1) => {
    ...
    for (let r = 0; r < gridM; r++) {
      for (let c = 0; c < gridN; c++) {
        ...
        const pidLabel =
          assignment !== undefined ? `PID ${assignment.programId}` : `Tile (${r},${c})`;
  ...
  // Step 2: Calculate total_programs (2)
  addStep(
    2,
  ...
  // Step 4: Calculate num_pid_in_group (5)
  addStep(
    5,
...
const TRITONL2CACHESWIZZLEDGEMMSCHEDULER_TRIVIA: TriviaMeta = {
  ...
  hints: [
    {
      line: 12,
      hint: "Triton swizzled row tile calculation: pid_m = first_pid_m + ((program_id % num_pid_in_group) % group_size_m_adj).",
    },
    {
      line: 13,
      hint: "Triton swizzled column tile calculation: pid_n = (program_id % num_pid_in_group) // group_size_m_adj.",
    },
  ],
  lineExplanations: {
    1: "Defines entry point for triton_l2_cache_swizzled_gemm_scheduler function implementing Triton Grouped Block Scheduling.",
    2: "Calculates total GPU thread block programs total_programs = grid_m * grid_n.",
    3: "Initializes empty list schedule to log swizzled program execution order.",
    4: "Blank line before group size calculation.",
    5: "Calculates total programs per L2 swizzled group num_pid_in_group = group_size_m * grid_n.",
    6: "Blank line before program iteration loop.",
    7: "Iterates over program_id from 0 to total_programs - 1.",
    8: "Calculates group index group_id = program_id // num_pid_in_group.",
    9: "Calculates first row tile index in group first_pid_m = group_id * group_size_m.",
    10: "Calculates boundary-adjusted group height group_size_m_adj = min(grid_m - first_pid_m, group_size_m).",
    11: "Blank line before swizzled tile index calculations.",
    12: "Calculates L2 swizzled row block coordinate pid_m = first_pid_m + ((program_id % num_pid_in_group) % group_size_m_adj).",
    13: "Calculates L2 swizzled column block coordinate pid_n = (program_id % num_pid_in_group) // group_size_m_adj.",
    14: "Blank line before appending to schedule.",
    15: "Appends tuple of (program_id, pid_m, pid_n) to schedule list.",
    16: "Blank line separating program loop from return statement.",
    17: "Returns completed schedule list of swizzled program tile assignments.",
  },
};
...
export const tritonL2CacheSwizzledGemmScheduler: AlgorithmDefinition<tritonL2CacheSwizzledGemmSchedulerInput> =
  {
    ...
    constraints: ["grid_m >= 1", "grid_n >= 1", "1 <= group_size_m <= 32"],
    examples: [
      {
        kind: "basic",
        ...
        outputDisplay:
          "Program ID Schedule: PID 0->(0,0), PID 1->(1,0), PID 2->(0,1), PID 3->(1,1)",
        input: DEFAULT_TRITONL2CACHESWIZZLEDGEMMSCHEDULER_INPUT,
        output: "Swizzled Schedule [(0,0,0), (1,1,0), (2,0,1), (3,1,1), ...]",
        explanation:
          "Swizzles 16 program IDs into 2-row column strips. Programs 0 and 1 execute (0,0) and (1,0) vertically, sharing matrix B column tile 0 in L2 cache.",
      },
    ],
      ...
      keyTerms: [
        {
          ...
          definition:
            "Reordering GPU thread block execution sequence to maximize L2 cache reuse of matrix operands.",
        },
        {
          term: "Grouped Block Scheduling",
          definition:
            "Triton tile scheduling strategy grouping GROUP_SIZE_M rows into vertical column execution strips.",
        },
        {
          term: "Program ID (pid)",
        ...
        },
        {
          term: "Cache Hit Rate",
          definition:
            "Percentage of memory accesses satisfied by fast GPU L2 cache (2 TB/s) rather than slow HBM DRAM.",
        },
      ],
    },
