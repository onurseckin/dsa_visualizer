...
export const FLASHATTENTION1FORWARDTILING_CODE = "def flash_attention1_forward_tiling(input_data: list) -> list:\n    # FlashAttention-1 SRAM Tiled Forward Kernel (Medium)\n    # Tiles Q, K, V matrices into SRAM to execute exact attention with O(N) memory traffic.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
const FLASHATTENTION1FORWARDTILING_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for FlashAttention-1 SRAM Tiled Forward Kernel.",
...
export const flashAttention1ForwardTiling: AlgorithmDefinition<flashAttention1ForwardTilingInput> = {
  ...
  topicGuide: {
    overview: "FlashAttention-1 tiles Q and K/V matrices into SRAM, reducing HBM reads from O(N^2) to O(N).",
    sections: [
      { heading: "Core Concept", body: "Tiles Q, K, V matrices into SRAM to execute exact attention with O(N) memory traffic." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"FlashAttention-1","definition":"IO-aware exact attention tiling in GPU SRAM."}],
  },
