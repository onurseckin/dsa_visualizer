        ...
        for i in range(0, N, Br):
            ...
            for r, q_vec in enumerate(Q_block):
                ...
                for col in range(d):
                    ...
                    O[row_idx][col] = (O[row_idx][col] * lse[row_idx] * scale_prev + pv_col) / l_new
...
const FLASHATTENTION1FORWARDTILING_TRIVIA: TriviaMeta = {
  skipLines: [4, 8, 12, 15, 19, 22, 25, 27, 28, 30, 33],
  distractors: [
    "S = Q @ K.T",
    "O = softmax(S) @ V",
  ...
  hints: [
    { line: 21, hint: "Online max updating equation: m_new = max(m[row_idx], m_curr)." },
    {
      line: 29,
      hint: "Online softmax output correction: O[row_idx][col] = (O * lse[row_idx] * scale_prev + pv_col) / l_new.",
    },
  ],
  lineExplanations: {
    ...
    4: "Blank line before output buffers allocation.",
    5: "Allocates output matrix O (N x d) filled with zeros.",
    6: "Allocates online normalizer accumulator list lse of size N.",
    7: "Allocates max score tracker list m of size N initialized to negative infinity.",
    8: "Blank line before outer column block loop.",
    9: "Iterates over column tile block index j from 0 to N in steps of Bc.",
    10: "Slices K_block = K[j : j + Bc] into SRAM shared memory.",
    11: "Slices V_block = V[j : j + Bc] into SRAM shared memory.",
    12: "Blank line before inner row block loop.",
    13: "Iterates over row tile block index i from 0 to N in steps of Br.",
    14: "Slices Q_block = Q[i : i + Br] into SRAM shared memory.",
    15: "Blank line before Q_block row iteration.",
    16: "Iterates over row index r and query vector q_vec in enumerate(Q_block).",
    17: "Calculates global sequence row index row_idx = i + r.",
    18: "Calculates scaled dot-product attention score block scores = Q_block * K_block^T * scale.",
    19: "Blank line before online max calculation.",
    20: "Calculates local block max score m_curr = max(scores).",
    21: "Updates online row max score m_new = max(m[row_idx], m_curr).",
    22: "Blank line before exponentiation.",
    23: "Calculates rescaled exponentiated scores exp_scores = [exp(s - m_new) for s in scores].",
    24: "Updates online normalizer sum l_new = lse[row_idx] * exp(m_prev - m_new) + sum(exp_scores).",
    25: "Blank line before output update loop.",
    26: "Calculates previous output rescaling factor scale_prev = exp(m_prev - m_new).",
    27: "Iterates over head dimension column col from 0 to d - 1.",
    28: "Computes matrix multiplication of exp_scores * V_block for column col.",
    29: "Rescales and updates FlashAttention output cell O[row_idx][col] = (O * lse[row_idx] * scale_prev + pv_col) / l_new.",
    30: "Blank line before updating state arrays.",
    31: "Persists updated row max score m[row_idx] = m_new.",
    32: "Persists updated row normalizer lse[row_idx] = l_new.",
    33: "Blank line separating tiling loops from return statement.",
    34: "Returns completed FlashAttention output matrix O.",
  },
};
...
export const flashAttention1ForwardTiling: AlgorithmDefinition<flashAttention1ForwardTilingInput> =
  {
    ...
    constraints: ["1 <= N <= 8192", "1 <= d <= 128", "1 <= Br, Bc <= 256"],
    examples: [
      {
        kind: "basic",
        ...
        output: "[[38.79, 48.79], [34.05, 44.05], [37.75, 47.75], [36.96, 46.96]]",
        explanation:
          "Tiles Q, K, V into 2x2 SRAM blocks, computing exact attention via online softmax with zero HBM N x N matrix materialization.",
      },
    ],
    ...
    complexityAnalysis: {
      ...
      space:
        "Requires $O(N \\cdot d)$ memory space for output $O$ and online trackers $m, lse$, eliminating $O(N^2)$ intermediate memory.",
    },
    topicGuide: {
      overview:
      ...
      keyTerms: [
        {
          term: "FlashAttention",
          definition:
            "Exact IO-aware fast attention algorithm utilizing SRAM tiling and online softmax (Dao et al. 2022).",
        },
        {
          term: "Online Softmax",
          definition:
            "Algorithm computing softmax incrementally over tiled blocks without storing full N x N matrix.",
        },
        {
          term: "SRAM Shared Memory",
          definition:
            "Ultra-fast 19 TB/s GPU on-chip memory buffer used to store Q, K, V tile blocks.",
        },
        {
          term: "HBM DRAM",
          definition:
            "Main GPU High Bandwidth Memory (2 TB/s) where model weights and final outputs reside.",
        },
      ],
    },
