...
export const generateTRITONMLIRTOPTXCOMPILERPIPELINESIMULATORSteps = (
  ...
  const getSnapshot = (activeStageIdx: number = -1) => {
...
const TRITONMLIRTOPTXCOMPILERPIPELINESIMULATOR_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 13, hint: "Stage 2: High-Level Triton IR (tt dialect) representing block tensors." },
    {
      line: 21,
      hint: "Stage 3: Target-Specific TritonGPU IR (tritongpu dialect) assigning warp layouts.",
    },
    { line: 32, hint: "Stage 5: Final NVIDIA PTX Assembly containing mma.sync instructions." },
  ],
  lineExplanations: {
    1: "Defines entry point for triton_mlir_to_ptx_compiler_pipeline_simulator function.",
    2: "Initializes empty dictionary pipeline to log intermediate representation (IR) strings.",
    3: "Blank line before Stage 1 Python AST.",
    4: "Assigns 1_python_ast IR string representing high-level @triton.jit function definition.",
    5: "@triton.jit decorator tag.",
    6: "Function signature for kernel_{operation_type}.",
    7: "High-level block load: a = tl.load(A_ptr).",
    8: "High-level block load: b = tl.load(B_ptr).",
    9: "High-level block matrix multiply: c = tl.dot(a, b).",
    10: "High-level block store: tl.store(C_ptr, c).",
    11: "Closing parenthesis for Stage 1 string.",
    12: "Blank line before Stage 2 Triton IR.",
    13: "Assigns 2_triton_ir IR string representing tt dialect block tensor operations.",
    14: "tt.load instruction for tensor A: tensor<block_m x block_n x f16>.",
    15: "tt.load instruction for tensor B: tensor<block_m x block_n x f16>.",
    16: "tt.dot instruction performing block matrix multiply: tt.dot %a, %b.",
    17: "Closing parenthesis for Stage 2 string.",
    18: "Blank line before thread calculations.",
    19: "Sets GPU warp thread constant threads_per_warp = 32.",
    20: "Calculates total CUDA threads total_threads = num_warps * 32.",
    21: "Assigns 3_tritongpu_ir IR string representing tritongpu dialect with explicit warp layouts.",
    22: "Defines #tritongpu.mma layout attribute with warpsPerCTA=[num_warps, 1].",
    23: "tritongpu.mma instruction assigned to specific GPU warp layout.",
    24: "Closing line for tritongpu.mma tensor type.",
    ...
    37: "Blank line before return statement.",
    38: "Returns dictionary pipeline containing all 5 compiler stage IR strings.",
  },
};
    ...
    examples: [
      {
        ...
        outputDisplay:
          "5 Compiler Stage IR Snippets (Python AST -> Triton IR -> TritonGPU IR -> LLVM IR -> PTX)",
        input: DEFAULT_TRITONMLIRTOPTXCOMPILERPIPELINESIMULATOR_INPUT,
        output: "Dictionary with 5 Stage IR strings",
        explanation:
          "Simulates the 5 lowering stages of Triton compiler, producing executable PTX assembly with mma.sync Tensor Core instructions targeting 128 CUDA threads.",
      },
    ],
      ...
      keyTerms: [
        {
          term: "MLIR Dialect",
          definition:
            "Custom domain-specific intermediate representation pass in MLIR (e.g. tt dialect, tritongpu dialect).",
        },
        {
          term: "Lowering",
          definition:
            "Process of transforming high-level compiler abstractions into progressively lower-level machine IR instructions.",
        },
        {
          term: "NVVM Intrinsics",
          definition:
            "LLVM compiler intrinsic functions representing low-level NVIDIA GPU hardware operations.",
        },
        {
          term: "PTX Assembly",
          definition:
            "NVIDIA Parallel Thread Execution virtual instruction set architecture compiled to native SASS GPU machine code.",
        },
      ],
    },
