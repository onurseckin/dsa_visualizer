...
export const segmentTree: AlgorithmDefinition<SegmentTreeInput> = {
  ...
  difficulty: "Hard",
  description:
    "<p>A <strong>Segment Tree</strong> structures an array into a balanced binary tree of nested intervals where leaves hold individual elements and internal nodes cache interval aggregates. It supports both range queries (e.g. sum, min, max) and point updates in <code>O(log N)</code> time per operation.</p><h3>State Representation</h3><p>The state is stored as a 4N binary tree where node <code>v</code> for interval <code>[start...end]</code> has children <code>2v</code> covering <code>[start...mid]</code> and <code>2v+1</code> covering <code>[mid+1...end]</code>.</p><h3>Input Parameters</h3><ul><li><code>array</code>: Initial numerical array.</li><li><code>operations</code>: Array of point update or range query operations.</li></ul><h3>Output</h3><ul><li><code>int / Array</code>: Query results and updated tree node state.</li></ul>",
