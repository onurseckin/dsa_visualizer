...
const FUSEDDEPTHWISESEPARABLECONV2DENGINE_TRIVIA: TriviaMeta = {
  skipLines: [6, 9, 11, 17, 18, 19, 20, 24, 27, 30],
  distractors: [
  ...
  lineExplanations: {
    ...
    2: "Measures number of input channels c_in from image array length.",
    3: "Measures input feature map spatial height h_in and width w_in.",
    4: "Measures depthwise spatial kernel height k_h and width k_w.",
    5: "Measures number of output channels c_out from pointwise kernel length.",
    6: "Blank line before output shape calculation.",
    ...
    9: "Blank line before output buffer allocation.",
    10: "Allocates 3D output feature tensor of shape (c_out, h_out, w_out) filled with zero floats.",
    11: "Blank line separating tensor allocation from fused spatial loop.",
    12: "Iterates over output spatial row coordinate r from 0 to h_out - 1.",
    13: "Iterates over output spatial column coordinate c from 0 to w_out - 1.",
    14: "Allocates local SRAM depthwise vector dw_vec of size c_in filled with zero floats.",
    15: "Iterates over input feature channel ch from 0 to c_in - 1.",
    16: "Resets depthwise spatial accumulator acc to 0.0 for channel ch.",
    17: "Iterates over depthwise kernel spatial row kr from 0 to k_h - 1.",
    18: "Iterates over depthwise kernel spatial column kc from 0 to k_w - 1.",
    ...
    21: "Checks if spatial coordinate (ir, ic) lies within valid image bounds.",
    22: "Multiplies channel pixel image[ch][ir][ic] by depthwise weight and accumulates into acc.",
    23: "Stores completed depthwise spatial scalar acc into dw_vec[ch].",
    24: "Blank line before Phase 2 Pointwise fusion.",
    25: "Iterates over output feature channel index co from 0 to c_out - 1.",
    26: "Resets pointwise accumulator pw_acc to 0.0 for output channel co.",
    27: "Iterates over input feature channel index ci from 0 to c_in - 1.",
    28: "Multiplies depthwise scalar dw_vec[ci] by pointwise weight and accumulates into pw_acc.",
    29: "Stores completed fused activation pw_acc into output tensor at (co, r, c).",
    30: "Blank line separating fused loops from return statement.",
...
export const fusedDepthwiseSeparableConv2dEngine: AlgorithmDefinition<fusedDepthwiseSeparableConv2dEngineInput> =
  {
    ...
    examples: [
      {
        ...
        explanation:
          "Fuses depthwise spatial filtering and 1x1 pointwise projection in SRAM registers.",
      },
    ],
    ...
    timeComplexity: {
      ...
      average:
        "O((C_{in} \\cdot K_h \\cdot K_w + C_{out} \\cdot C_{in}) \\cdot H_{out} \\cdot W_{out})",
      worst:
        "O((C_{in} \\cdot K_h \\cdot K_w + C_{out} \\cdot C_{in}) \\cdot H_{out} \\cdot W_{out})",
    },
    ...
    complexityAnalysis: {
      ...
      space:
        "Requires $O(C_{out} H_{out} W_{out})$ memory for output feature map storage; intermediate depthwise vectors are held in $O(C_{in})$ SRAM register space.",
    },
    topicGuide: {
      overview:
      ...
      keyTerms: [
        ...
        },
        {
          ...
          definition:
            "1x1 convolution computing linear combinations across channels (C_in -> C_out).",
        },
        {
        ...
        },
        {
          term: "FLOP Reduction",
          definition:
            "Complexity reduction ratio ~1/9 achieved by depthwise separable factorization.",
        },
      ],
    },
