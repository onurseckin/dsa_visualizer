...
export const generateConv2dToGemmReceptiveFieldUnrollSteps = (
  ...
  const getSnapshot = (currentPatchIdx: number = -1): MatrixVisualSnapshot => {
    const rows = Math.max(im2colMatrix.length, 1);
    const cols = patchDim;
    const cells: MatrixCellItem[] = [];

    const rowHeaders: string[] = [];
    const colHeaders: string[] = Array.from({ length: cols }, (_, i) => `Tap ${i}`);

    if (im2colMatrix.length === 0) {
      rowHeaders.push("Patch 0 (Pending)");
  ...
  for (let r = 0; r < hOut; r++) {
    ...
    for (let c = 0; c < wOut; c++) {
      ...
      for (let kr = 0; kr < kH; kr++) {
        addStep(17, `Kernel Row Loop: kr = ${kr}`, `Scanning kernel row kr = ${kr} of ${kH - 1}.`, {
          r,
          c,
          kr,
        });
...
const CONV2DTOGEMMRECEPTIVEFIELDUNROLL_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    ...
    4: "Blank line separates input-shape setup from padded-buffer allocation.",
    5: "Allocates zero-padded image matrix buffer.",
    6: "Iterates over input image row index r from 0 to h_in - 1.",
    7: "Iterates over input image column index c from 0 to w_in - 1.",
    8: "Copies input pixel image[r][c] into padded matrix with padding offset.",
    9: "Blank line separates padding construction from output-shape calculation.",
    10: "Calculates spatial output height h_out using integer division floor.",
    11: "Calculates spatial output width w_out using integer division floor.",
    12: "Blank line separates output dimensions from im2col matrix initialization.",
    13: "Initializes empty list im2col_matrix to accumulate unrolled 1D patch vectors.",
    14: "Iterates over output spatial row coordinate r from 0 to h_out - 1.",
    15: "Iterates over output spatial column coordinate c from 0 to w_out - 1.",
    16: "Initializes empty list patch for current spatial receptive field window.",
    17: "Iterates over filter kernel spatial row tap kr from 0 to k_h - 1.",
    18: "Iterates over filter kernel spatial column tap kc from 0 to k_w - 1.",
    19: "Appends padded pixel padded[r * stride + kr][c * stride + kc] to 1D patch vector.",
    20: "Appends completed 1D patch vector to im2col_matrix list.",
    21: "Blank line separates patch extraction from the function return value.",
    22: "Returns unrolled im2col_matrix and shape metadata dictionary.",
  },
};
...
export const conv2dToGemmReceptiveFieldUnroll: AlgorithmDefinition<conv2dToGemmReceptiveFieldUnrollInput> =
  {
    ...
    constraints: ["1 <= H_in, W_in <= 512", "1 <= K_h, K_w <= 11", "stride >= 1", "padding >= 0"],
    examples: [
      {
        kind: "basic",
        ...
        explanation:
          "Extracts 9 spatial 2x2 patches, unrolling each patch into a 4-element row vector.",
      },
    ],
    ...
    topicGuide: {
      ...
      keyTerms: [
        ...
        },
        {
          ...
          definition:
            "Memory inflation ratio K_h * K_w resulting from unrolling overlapping spatial windows.",
        },
        {
          term: "GEMM Lowering",
          definition:
            "Executing convolution as matrix multiplication using high-performance BLAS libraries.",
        },
      ],
    },
