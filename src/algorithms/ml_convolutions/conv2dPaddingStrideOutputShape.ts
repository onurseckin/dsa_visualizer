...
export const CONV2DPADDINGSTRIDEOUTPUTSHAPE_CODE = "def conv2d_padding_stride_output_shape(input_data: list) -> list:\n    # 2D Conv Output Shape Calculator (Easy)\n    # Computes O = floor((I + 2P - K) / S) + 1.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const conv2dPaddingStrideOutputShape: AlgorithmDefinition<conv2dPaddingStrideOutputShapeInput> = {
  ...
  topicGuide: {
    overview: "Spatial convolution output dimensions depend on image size I, padding P, kernel size K, and stride S.",
    sections: [
