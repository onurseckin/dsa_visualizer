...
export const generateConv2dPaddingStrideOutputShapeSteps = (
  ...
  const getSnapshot = (stage: string, highlightRow: number = -1): MatrixVisualSnapshot => {
    const rows = 5;
    const cols = 4;
    const cells: MatrixCellItem[] = [
      {
        ...
        value: "h_in",
        label: "Input Height",
        state: highlightRow === 0 ? "active" : "default",
      },
      {
        row: 0,
        col: 1,
...

      {
        ...
        value: "k_h",
        label: "Kernel Height",
        state: highlightRow === 1 ? "active" : "default",
      },
      {
        row: 1,
        col: 1,
...

      {
        ...
        value: "pad_h",
        label: "Pad Height",
        state: highlightRow === 2 ? "active" : "default",
      },
      {
        row: 2,
        col: 1,
...

      {
        ...
        value: "stride_h",
        label: "Stride H",
        state: highlightRow === 3 ? "active" : "default",
      },
      {
        row: 3,
        col: 1,
...

      {
        ...
        value: "dilation_h",
        label: "Dilation H",
        state: highlightRow === 4 ? "active" : "default",
      },
      {
        row: 4,
        col: 1,
...
const CONV2DPADDINGSTRIDEOUTPUTSHAPE_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 2, hint: "Dilation expands kernel size: eff_k = k + (k - 1) * (dilation - 1)." },
    { line: 5, hint: "Spatial output height formula: (h_in + 2*pad_h - eff_k_h) // stride_h + 1." },
  ],
  lineExplanations: {
    1: "Defines entry point for 2D Conv Output Shape Calculator function.",
    2: "Calculates effective vertical kernel size eff_k_h factoring in dilation_h.",
    3: "Calculates effective horizontal kernel size eff_k_w factoring in dilation_w.",
    ...
    7: "Blank line before validity check.",
    8: "Evaluates boolean validity check ensuring both h_out and w_out are greater than zero.",
    9: "Opens shape metadata dictionary return block.",
    10: "Stores clamped output height max(0, h_out) in dictionary under key 'h_out'.",
    11: "Stores clamped output width max(0, w_out) in dictionary under key 'w_out'.",
    12: "Stores vertical effective kernel footprint eff_k_h in dictionary under key 'eff_kernel_h'.",
    13: "Stores horizontal effective kernel footprint eff_k_w in dictionary under key 'eff_kernel_w'.",
    14: "Stores boolean spatial validity flag in dictionary under key 'is_valid_shape'.",
    15: "Closes metadata dictionary return block.",
  },
};
...
export const conv2dPaddingStrideOutputShape: AlgorithmDefinition<conv2dPaddingStrideOutputShapeInput> =
  {
    ...
    examples: [
      ...
      },
      {
        ...
        output:
          "{ h_out: 106, w_out: 106, eff_kernel_h: 13, eff_kernel_w: 13, is_valid_shape: true }",
