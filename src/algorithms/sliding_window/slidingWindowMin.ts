...
const SLIDING_WINDOW_MIN_TRIVIA: TriviaMeta = {
  lineExplanations: {
    ...
    8: 'Comment marking the eviction step: candidates that have slid outside the window on the left need to be dropped before anything else.',
    9: 'Checks whether the index at the front of the deque has fallen out of the window — a window of size k ending at i starts at i - k + 1, so anything at or before i - k is stale.',
    10: 'Removes the stale front index with popleft, since it can never be part of the current or any future window.',
    12: 'Comment marking the domination step: any older index whose value is no better than the incoming one is now permanently useless.',
    13: "Checks whether the value at the back of the deque is greater than or equal to nums[i] — if so, that older candidate can never win a window again, since nums[i] is at least as small and will outlive it in the window.",
    14: 'Pops the dominated index off the back, since a newer, at-least-as-small value has just made it irrelevant.',
    16: "Appends the current index i to the back of the deque, now that everything it would dominate has been cleared out — the deque's values stay increasing from front to back.",
    18: "Comment marking the read step: once the window has grown to size k, its minimum is ready to be read off.",
    19: 'Checks whether i has reached at least k - 1, the first index at which a full window of size k exists.',
    20: "Reads nums[dq[0]] — the front of the deque is guaranteed to be the smallest value still inside the window — and appends it to result without rescanning the window.",
    22: 'Returns the completed list of per-window minimums, one entry for every window the scan passed over.',
  },
};
