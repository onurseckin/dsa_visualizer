...
export const MULTIQUERYATTENTIONBROADCAST_CODE = "def multi_query_attention_broadcast(input_data: list) -> list:\n    # Multi-Query Attention (MQA) Broadcaster (Medium)\n    # Shares a single Key/Value head across all Query heads to minimize KV-cache VRAM.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
export const multiQueryAttentionBroadcast: AlgorithmDefinition<multiQueryAttentionBroadcastInput> = {
  ...
  description: "Shares a single Key/Value head across all Query heads to minimize KV-cache VRAM.",
  ...
  topicGuide: {
    ...
    sections: [
      { heading: "Core Concept", body: "Shares a single Key/Value head across all Query heads to minimize KV-cache VRAM." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"Multi-Query Attention","definition":"MQA uses 1 shared KV head across all H query heads."}],
  },
