...
export const generateSlidingWindowPrefixAttentionEngineSteps = (
  ...
  for (let i = 0; i < qLen; i++) {
    ...
    for (let j = 0; j < kvLen; j++) {
      ...
      addStep(
        16,
        `Evaluate Mask Branch Condition: is_prefix or is_window -> ${isPrefix} or ${isWindow} = ${canAttend}`,
        canAttend
          ? `At least one visibility condition met for (Q${i}, K${j}). Branching to append True.`
          : `Neither prefix nor window condition met for (Q${i}, K${j}). Branching to append False.`,
        { i, j, isPrefix, isWindow, canAttend },
        i,
        j,
      );
...
const SLIDINGWINDOWPREFIXATTENTIONENGINE_TRIVIA: TriviaMeta = {
  skipLines: [2, 4, 5, 6, 7, 8, 15, 18, 21],
  distractors: [
  ...
  hints: [
    { line: 13, hint: "Check if key index j is in static prefix region j < prefix_len." },
    { line: 14, hint: "Check if key index j is in local sliding window i - window_size < j <= i." },
    { line: 16, hint: "Check if either prefix condition or sliding window condition holds." },
  ],
  lineExplanations: {
    ...
    9: "Initializes list container for collecting 2D boolean mask rows.",
    10: "Iterates over query token index i from 0 to q_len - 1.",
    11: "Initializes empty list container for current query row.",
    12: "Iterates over key-value token index j from 0 to kv_len - 1.",
    13: "Computes boolean is_prefix = (j < prefix_len) for static prefix check.",
    14: "Computes boolean is_window = (i - window_size < j <= i) for sliding window check.",
    15: "Empty whitespace separator line.",
    16: "Checks if either is_prefix or is_window condition is True.",
    17: "Appends True to row (valid unmasked attention position).",
    18: "Else branch for masked attention position.",
    19: "Appends False to row (masked position filled with -inf in logit tensor).",
    20: "Appends completed boolean mask row to mask matrix.",
    21: "Empty whitespace separator line.",
    22: "Returns computed 2D boolean mask matrix.",
  },
};
