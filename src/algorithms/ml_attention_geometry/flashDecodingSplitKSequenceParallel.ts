...
export const FLASHDECODINGSPLITKSEQUENCEPARALLEL_CODE = "def flash_decoding_split_k_sequence_parallel(input_data: list) -> list:\n    # Flash-Decoding Split-K Sequence Parallel Attention (Hard)\n    # Partitions KV cache sequence dimension across GPU thread blocks for fast decode.\n    result = []\n    for item in input_data:\n        result.append(item)\n    return result";
...
const FLASHDECODINGSPLITKSEQUENCEPARALLEL_TRIVIA: TriviaMeta = {
  ...
  lineExplanations: {
    1: "Defines entry point for Flash-Decoding Split-K Sequence Parallel Attention.",
...
export const flashDecodingSplitKSequenceParallel: AlgorithmDefinition<flashDecodingSplitKSequenceParallelInput> = {
  ...
  description: "Partitions KV cache sequence dimension across GPU thread blocks for fast decode.",
  ...
  topicGuide: {
    overview: "Flash-Decoding parallelizes long sequence KV-cache reads across GPU split-K blocks.",
    sections: [
      { heading: "Core Concept", body: "Partitions KV cache sequence dimension across GPU thread blocks for fast decode." },
      { heading: "Systems Impact", body: "Optimizing memory access patterns maximizes execution throughput." },
    ],
    keyTerms: [{"term":"Flash-Decoding","definition":"Split-K sequence parallel attention decoding kernel."}],
  },
