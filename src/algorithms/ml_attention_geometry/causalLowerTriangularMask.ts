...
export const generateCausalLowerTriangularMaskSteps = (
  ...
  const qSeqLen = Math.max(input.qSeqLen ?? (input.data?.length || 4), 1);
  const kvSeqLen = Math.max(input.kvSeqLen ?? (input.data?.length || 4), 1);
  ...
  addStep(
    ...
    "Setting up query sequence length q_seq_len and key sequence length kv_seq_len for autoregressive attention mask generation.",
  ...
  for (let i = 0; i < qSeqLen; i++) {
    addStep(
      ...
      `Processing causal constraint checks for query token Q${i} across key sequence.`,
      { i, q_seq_len: qSeqLen },
      i,
      undefined,
    );
...
const CAUSALLOWERTRIANGULARMASK_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 6, hint: "Check if key index j is less than or equal to query index i." },
    { line: 7, hint: "Assign 0.0 to unmasked causal positions." },
    { line: 9, hint: "Assign -inf to future positions to prohibit lookahead attention." },
  ],
  lineExplanations: {
    1: "Defines entry point for Causal Lower-Triangular Mask Generator function.",
    2: "Initializes outer matrix structure to hold mask rows.",
    3: "Iterates over query sequence token index i from 0 to q_seq_len - 1.",
    4: "Initializes empty row list for current query token i.",
    5: "Iterates over key sequence token index j from 0 to kv_seq_len - 1.",
    6: "Evaluates causal validity condition: is key index j <= query index i?",
    7: "Appends 0.0 logit offset allowing query token i to attend to past/present key j.",
    8: "Branch executed when key index j > query index i (future token).",
    9: "Appends -inf logit offset preventing query token i from attending to future key j.",
    10: "Appends completed query mask row to the mask matrix tensor.",
    11: "Returns final N x M lower-triangular causal attention mask matrix.",
  },
};
