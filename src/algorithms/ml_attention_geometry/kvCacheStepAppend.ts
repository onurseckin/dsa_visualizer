...
export const generateKvCacheStepAppendSteps = (input: kvCacheStepAppendInput): AlgorithmStep[] => {
  ...
  for (let t = 0; t < stepsToAppend; t++) {
    ...
    addStep(
      10,
      `Advance Sequence Pointer: updated_seq_len = ${currentSeqLen + 1}`,
...
const KVCACHESTEPAPPEND_TRIVIA: TriviaMeta = {
  ...
  hints: [
    { line: 8, hint: "Assign new key vector directly into k_cache[current_seq_len]." },
    { line: 9, hint: "Assign new value vector directly into v_cache[current_seq_len]." },
    { line: 10, hint: "Increment current_seq_len by 1." },
    { line: 11, hint: "Return updated k_cache, v_cache, and updated_seq_len." },
  ],
  lineExplanations: {
    1: "Defines entry point function for autoregressive KV cache step append operations.",
    2: "Specifies type annotation for key cache matrix buffer.",
    3: "Specifies type annotation for value cache matrix buffer.",
    ...
    8: "Overwrites key cache slot at index current_seq_len with new_k vector.",
    9: "Overwrites value cache slot at index current_seq_len with new_v vector.",
    10: "Increments active sequence length counter by 1.",
    11: "Returns updated key/value cache buffer references and incremented length.",
  },
};
