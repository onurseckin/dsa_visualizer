import type { CourseTopicJourney } from "../../../courseTypes";
import { chapter_1 } from "./chapter_01_foundations";
import { chapter_2 } from "./chapter_02_practice_laboratory";

export const ml_continuous_batching_orca_journey: CourseTopicJourney = {
  id: "ml_continuous_batching_orca",
  trackId: "machine-learning",
  title: "CONTINUOUS BATCHING ORCA",
  subtitle: "University-Grade Masterclass Depth",
  icon: "Cpu",
  difficulty: "Hard",
  estimatedMinutes: 90,
  chapters: [chapter_1, chapter_2],
};

export const ml_continuous_batching_orca = ml_continuous_batching_orca_journey;
export const topic = ml_continuous_batching_orca_journey;
