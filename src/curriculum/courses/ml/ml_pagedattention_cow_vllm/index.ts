import type { CourseTopicJourney } from "../../../courseTypes";
import { chapter_1 } from "./chapter_01_foundations";
import { chapter_2 } from "./chapter_02_practice_laboratory";

export const ml_pagedattention_cow_vllm_journey: CourseTopicJourney = {
  id: "ml_pagedattention_cow_vllm",
  trackId: "machine-learning",
  title: "PAGEDATTENTION COW VLLM",
  subtitle: "University-Grade Masterclass Depth",
  icon: "Cpu",
  difficulty: "Hard",
  estimatedMinutes: 90,
  chapters: [chapter_1, chapter_2],
};

export const ml_pagedattention_cow_vllm = ml_pagedattention_cow_vllm_journey;
export const topic = ml_pagedattention_cow_vllm_journey;
