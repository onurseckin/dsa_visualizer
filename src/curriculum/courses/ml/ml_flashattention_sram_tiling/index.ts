import type { CourseTopicJourney } from "../../../courseTypes";
import { chapter_1 } from "./chapter_01_foundations";
import { chapter_2 } from "./chapter_02_practice_laboratory";

export const ml_flashattention_sram_tiling_journey: CourseTopicJourney = {
  id: "ml_flashattention_sram_tiling",
  trackId: "machine-learning",
  title: "FlashAttention & SRAM Tiling",
  subtitle: "University-Grade Masterclass Depth",
  icon: "Cpu",
  difficulty: "Hard",
  estimatedMinutes: 90,
  chapters: [chapter_1, chapter_2],
};

export const ml_flashattention_sram_tiling = ml_flashattention_sram_tiling_journey;
export const topic = ml_flashattention_sram_tiling_journey;
