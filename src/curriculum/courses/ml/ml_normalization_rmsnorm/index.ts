import type { CourseTopicJourney } from "../../../courseTypes";
import { chapter_1 } from "./chapter_01_foundations";
import { chapter_2 } from "./chapter_02_practice_laboratory";

export const ml_normalization_rmsnorm_journey: CourseTopicJourney = {
  id: "ml_normalization_rmsnorm",
  trackId: "machine-learning",
  title: "NORMALIZATION RMSNORM",
  subtitle: "University-Grade Masterclass Depth",
  icon: "Cpu",
  difficulty: "Hard",
  estimatedMinutes: 90,
  chapters: [chapter_1, chapter_2],
};

export const ml_normalization_rmsnorm = ml_normalization_rmsnorm_journey;
export const topic = ml_normalization_rmsnorm_journey;
