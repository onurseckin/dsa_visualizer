import type { CourseTopicJourney } from "../../../courseTypes";
import { chapter_1 } from "./chapter_01_foundations";
import { chapter_2 } from "./chapter_02_practice_laboratory";

export const ml_mlp_backpropagation_journey: CourseTopicJourney = {
  id: "ml_mlp_backpropagation",
  trackId: "machine-learning",
  title: "MLP BACKPROPAGATION",
  subtitle: "University-Grade Masterclass Depth",
  icon: "Cpu",
  difficulty: "Hard",
  estimatedMinutes: 90,
  chapters: [chapter_1, chapter_2],
};

export const ml_mlp_backpropagation = ml_mlp_backpropagation_journey;
export const topic = ml_mlp_backpropagation_journey;
