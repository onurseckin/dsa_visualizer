import type { CourseTopicJourney } from "../../../courseTypes";
import { chapter_1 } from "./chapter_01_foundations";
import { chapter_2 } from "./chapter_02_practice_laboratory";

export const ml_attention_causal_sdpa_journey: CourseTopicJourney = {
  id: "ml_attention_causal_sdpa",
  trackId: "machine-learning",
  title: "ATTENTION CAUSAL SDPA",
  subtitle: "University-Grade Masterclass Depth",
  icon: "Cpu",
  difficulty: "Hard",
  estimatedMinutes: 90,
  chapters: [chapter_1, chapter_2],
};

export const ml_attention_causal_sdpa = ml_attention_causal_sdpa_journey;
export const topic = ml_attention_causal_sdpa_journey;
