import type { CourseTopicJourney } from "../../../courseTypes";
import { chapter_1 } from "./chapter_01_foundations";
import { chapter_2 } from "./chapter_02_practice_laboratory";

export const ml_floating_point_kahan_journey: CourseTopicJourney = {
  id: "ml_floating_point_kahan",
  trackId: "machine-learning",
  title: "FLOATING POINT KAHAN",
  subtitle: "University-Grade Masterclass Depth",
  icon: "Cpu",
  difficulty: "Hard",
  estimatedMinutes: 90,
  chapters: [chapter_1, chapter_2],
};

export const ml_floating_point_kahan = ml_floating_point_kahan_journey;
export const topic = ml_floating_point_kahan_journey;
