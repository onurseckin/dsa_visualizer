...
markdown_content = """# Domain 1: Tensor Representation and Numerical Kernels
...
1. Input: `values=[1.0, 1e-16, 1e-16]`
   Output: `1.0` (Note: in pure FP64 naive sum might drop the tiny values if added sequentially, but Kahan accumulates them if they eventually sum to a representable difference. However, $10^{-16}$ in FP64 is near epsilon. Wait, example needs to be precise.)
...
**Constraints:** `0 <= len(x) <= 10^5`, `bit_width` is typically 8. $q_{min} = -2^{b-1}$, $q_{max} = 2^{b-1} - 1$ for signed. (Assume signed INT8 for bit_width=8). Wait, asymmetric often targets unsigned 0 to 255. Let's explicitly define $q_{min}=0, q_{max}=2^{b}-1$ for unsigned asymmetric quantization.
...
os.makedirs('/Users/onurseckinsenoglu/repos/dsa_visualizer/research/ml-infra-curriculum/next-research/agentic-runs/2026-07-29-curriculum-v4-repair/rounds/round-01', exist_ok=True)
with open('/Users/onurseckinsenoglu/repos/dsa_visualizer/research/ml-infra-curriculum/next-research/agentic-runs/2026-07-29-curriculum-v4-repair/rounds/round-01/domain-01-tensors-and-numerics.md', 'w') as f:
